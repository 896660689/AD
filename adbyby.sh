#!/bin/sh
## Adaptation Grassland in Lucheng 2019.1.9
...
if [ -f "$ad_home/bin/adbyby" ]; then
	...
	logger -t " $LOGTIME "" adbyby 找到$port个8118透明代理端口,正在关闭..."
	while [[ "$port" -ge 1 ]]
	do
	...
	logger -t " $LOGTIME "" adbyby 进程已成功关闭..."
	...
	if [ -f "$ad_home/bin/adhook.ini" ]; then
		...
		if [ "$ad_whost" != "" ] ; then
			logger -t " $LOGTIME "" adbyby ""添加过滤白名单地址"
	...
	if [ "$check" = 0 ]; then
		logger -t " $LOGTIME "" adbyby ""启动失败。"
		exit 0
	else
		iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-ports 8118
		logger -t " $LOGTIME "" adbyby ""进程守护已启动。"
	fi
else
